 ...
 * so using signed instead.
 */
#define strtoul (unsigned)strtol
#endif
...
t_stat ex_reg (int flag, int32 sw, REG *rptr);
t_stat dep_reg (int flag, int32 sw, char *cptr, REG *rptr);
t_stat ex_addr (int flag, int32 sw, t_addr addr, DEVICE *dptr, UNIT *uptr);
t_stat dep_addr (int flag, int32 sw, char *cptr, t_addr addr, DEVICE *dptr,
...
{
	/* Jump to 160000 on startup if in PRO mode */

	sprintf(cbuf, "g 160000");
...
cptr = get_glyph (cptr, gbuf, 0);			/* get name */
...
#ifdef PRO
/* Don't reset PRO peripherals due to a RESET instruction */
...
t_stat exdep_reg_loop (int flag, int32 sw, char *ptr, REG *lptr, REG *hptr);
...
t_stat exdep_reg_loop (int flag, int32 sw, char *cptr, REG *lowr, REG *highr)
{
t_stat reason;
REG *rptr;
...
errno = 0;
mask = (k1 << rptr -> width) - 1;
val = get_uint (cptr, rptr -> radix, mask, &r);
...
if ((reason = print_sym (addr, sim_eval, uptr, sw)) > 0)
    reason = print_val (sim_eval[0], dptr -> dradix, dptr -> dwidth, PV_RZRO);
if (flag & EX_I) printf ("	");
else printf ("\n");
return reason;
}
...
if ((reason = parse_sym (cptr, addr, uptr, sim_eval, sw)) > 0) {
	sim_eval[0] = get_uint (cptr, dptr -> dradix, mask, &reason);
	if (reason != SCPE_OK) return reason;  }
count = 1 - reason;
